...
contract Medicine {
    ...
    function pickPackageDistributor(address _shipper) public {
        require(
            _shipper == shipper,
            "The package can only be picked by the shipper!"
        );
        require(
            status == medicinestatus(0),
            "the package left the manufacturer"
        );
    ...
    function receivePackageDistributor(address _retailer) public {
        require(
            ...
            "only distributor can receive the package"
        );
    ...
    function pickPackageRetailer(address _shipper) public {
        ...
        require(
            status == medicinestatus(2),
            "the package left the distributor"
        );
